...
def create_net(input_features, hidden_units, non_linearity, output_size):
    ...
    # keep track of the current number of neurons in current layer
    curr_num = input_features
    
    # add the hidden layers
    for i in range(len(hidden_units)):

        # retreive ith function and number of neurons for ith layer
        num_neurons = hidden_units[i]
        func = non_linearity[i]

        # add neurons to network
        net.add_module(f"fc_{i}", FullyConnectedLayer(curr_num, num_neurons))
        net.add_module(f"{func}_{i}", GeneralizedLogisticLayer(func))
        curr_num = num_neurons
        
    # add output layer
    net.add_module("predictions", FullyConnectedLayer(curr_num, output_size))
